...
- **TLS is proven end-to-end** — a new `scripts/checks/tls.sh` covers the half of the built-in-TLS
...
- **`mage testShort`** — runs the suite with `-short`, skipping the 5k-chunk migration load test, for a
  fast inner loop. `mage test` is unchanged and still proves the load claim on every CI push; until now
  the `testing.Short()` guard could not be reached through mage at all.
- **`internal/cogerr` has tests** — the package every other test asserts *through* (`cogerr.Is`) now
  covers its own contract: the Kind strings the MCP layer maps to tool errors, the unknown-Kind default,
