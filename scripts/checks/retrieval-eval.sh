...
go test ./internal/query/retrievaleval/ -v -timeout 30m \
  -run 'TestVectorLegCapacity|TestVectorLegRecallVsExact|TestFusedTopKUnderCorrectedScan|TestRecordExactProbePlan|TestKeywordORFallbackSweep|TestNoteLevelMembershipSweep|TestDiversityRerankSweep|TestGraphWeightSweep|TestGraphLegContribution' \
  || fail "retrieval evaluation sweeps failed"

pass "capacity, recall-vs-exact, fused-overlap, ground-truth plan, OR-fallback, note-level-membership, diversity-rerank, graph-weight and graph-contribution sweeps recorded"
